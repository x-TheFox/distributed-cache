...
std::string MemcachedProtocol::parseCommand(const std::string& request) {
    size_t spacePos = request.find(' ');
    return (spacePos == std::string::npos) ? request : request.substr(0, spacePos);
}

void MemcachedProtocol::handleGet(const std::string& request, std::string& response) {
...
void MemcachedProtocol::handleSet(const std::string& request, std::string& response) {
...
void MemcachedProtocol::processRequest(const std::string& request, std::string& response) {

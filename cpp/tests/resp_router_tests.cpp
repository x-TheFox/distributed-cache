...
TEST(RespRouter, MovedForRemoteKey) {
    // Build ring of two nodes
    ConsistentHash ring(10);
    ring.add_node("nodeA");
    ring.add_node("nodeB");

    LocalNodeInfo local{"nodeA", "127.0.0.1", 6384};
    auto router = std::make_shared<Router>(ring, local);
    // set node addresses
    router->add_node_address("nodeA", "127.0.0.1", 6384);
    router->add_node_address("nodeB", "127.0.0.1", 6385);

    Router::set_default(router);
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        if (ring.get_node(k) == "nodeB") { remote_key = k; break; }
    }
    ASSERT_FALSE(remote_key.empty());

    std::string setreq = "*3\r\n$3\r\nSET\r\n$" + std::to_string(remote_key.size()) + "\r\n" + remote_key + "\r\n$1\r\nv\r\n";
    auto r = send_recv_port(6384, setreq);
    EXPECT_NE(r.find("-MOVED"), std::string::npos);
    EXPECT_NE(r.find("127.0.0.1:6385"), std::string::npos);

    // pick a key that maps to nodeA (local)
    std::string local_key;
    for (int i = 0; i < 1000; ++i) {
        std::string k = "lkey" + std::to_string(i);
        if (ring.get_node(k) == "nodeA") { local_key = k; break; }
    }
